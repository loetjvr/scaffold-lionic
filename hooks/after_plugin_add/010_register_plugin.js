  ...
  if (plugin.indexOf('/') !== -1) {
    try {
      ...
      if (!fs.existsSync(pluginXmlPath)) {
        console.log(['There was no plugin.xml file found for path: ',
          pluginXmlPath].join(''));
      ...
      console.log('There was an error retrieving the plugin.xml filr ' +
        'from the 010_register_plugin.js hook', ex);
    }
  } else {
    pluginToAdd = plugin;
  }

  if (typeof pluginToAdd === 'string' &&
    packageJSON.cordovaPlugins.indexOf(pluginToAdd) === -1) {
    ...
    packageJSON.cordovaPlugins.forEach(function(checkPlugin) {
      if (typeof checkPlugin === 'object' &&
        checkPlugin.id === pluginToAdd.id) {
        pluginExists = true;
      }
    });
